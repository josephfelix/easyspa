	...
	)
{
	...
	{
		$state.go('app.usuario.perfil', {id: id});
	}
			...
			if ( json.status == 'OK' )
			{
				...
				for ( var ind in funcionarias )
				{
					...
					markerFuncionaria = new google.maps.Marker({
						position: latlng,
						map: map,
						title: funcionarias[ind].titulo,
						icon: 'img/map-easyspa.png'
					});
					$rootScope.markers.push( markerFuncionaria );
					contentStringFuncionaria = "<div class='pointer' ng-click='verPerfil(\""+funcionarias[ind].idanuncio+"\")'><a href='#' class='nome-funcionaria'>"+funcionarias[ind].titulo+"</a><span class='categoria-funcionaria'>"+categories[funcionarias[ind].categoria]+"</span></div>";
