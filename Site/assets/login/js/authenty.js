...
jQuery(function($) {
    ...
    $('.nav-step-btn').on('click', function (e) {
        if ($(this).parent().attr('id') === "next") {
            ...
            if (current.index() < 3) {
                showHideWizardPage(current.next(), navstep.next(), current);
            }
        } else if ($(this).parent().attr('id') === "prev") {
            e.preventDefault();
            if (current.prev().length === 0) return;

            showHideWizardPage(current.prev(), navstep.prev(), false);
        }
    });

    function showHideWizardPage(p, n, anterior) {
		var success = true;
		if ( anterior )
			if ( anterior.data('submit') )
				success = eval(anterior.data('submit'));
		if ( success )
		{
